...
{  
    ...
    {
			// Очистка очереди путем передачи накопленных данных в буффер
			  while((++length < STDOUT_QUEUE_SIZE) && 
            (xQueueReceive(stdout_queue, &buf[length], 0) == pdPASS));
        /* Отправка данных из буффера по UART */
				while(i!=length)
				{
